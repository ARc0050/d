...
public class tileMapScript : MonoBehaviour    
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(1))
        {
            ...
            {
                if (selectedUnit.GetComponent<UnitScript>().movementQueue.Count == 0 && ().combatQueue.Count==0selectedUnit.GetComponent<UnitScript>)//如果选中单位不在移动不在战斗
                {
    ...
    public void highlightEnemiesInRange(HashSet<Node> enemiesToHighlight)//高亮显示可攻击的节点
    {
        ...
        {
            quadOnMap[n.x, n.y].GetComponent<Renderer>().material = UIMat;
            quadOnMap[n.x, n.y].GetComponent<MeshRenderer>().enabled = true;
        }
    }
